...
price1 = []
ave_price1 = []
for s in range(0, 294):
    if data1['transactions'][s] != {}:
        for t in range(3):
...
fig, ax = plt.subplots()
ax.plot(price1, label='P2P_Market', color='blue', linestyle='-', marker='o')
ax.plot(price2, label='Filip', color='red', linestyle='--', marker='s')
ax.legend()
ax.set_ylabel('price of 4 buildings in every hour')
ax.set_title('The price of every building in 294h')
plt.show()