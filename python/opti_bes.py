...
def compute(node, params, par_rh, building_param, scenario, init_val, n_opt):
    """
    Parameters
    ----------
    eco : dict
    ...
    try:
        ...
        # TES
        for t in time_steps:
            model.addConstr(soc["tes"][t] <= soc_nom["tes"],
                            name="max_cap_tes_" + str(t))
            model.addConstr(soc["tes"][t] >= node["devs"]["tes"]["min_soc"] * soc_nom["tes"],
                            name="min_cap_" + str(t))
        # BAT
        for t in time_steps:
            # Guarantee feed OR load of BAT
        ...
        for t in time_steps:
            ...
            model.addConstr(p_ch[dev][t] == eta_ch * (heat["chp"][t] + heat["hp35"][t]
                                                      + heat["hp55"][t]  + heat["boiler"][t]),
        ...
        for t in time_steps:
            # Abbreviations
            dT_relative = (node["devs"]["hp35"]["dT_max"] * node["devs"]["hp35"]["exists"]
                           + node["devs"]["hp55"]["dT_max"] * node["devs"]["hp55"]["exists"]) \
                          / node["devs"]["tes"]["dT_max"]
            # Residual storage content
            #resSC = (node["devs"]["tes"]["volume_max"] * node["devs"]["tes"]["dT_max"]
            #         * params["phy"]["rho_w"] * params["phy"]["c_w"] * (1 - dT_relative) / 3600000)
            resSC = (node["devs"]["tes"]["cap"] * node["devs"]["tes"]["dT_max"] * (1 - dT_relative) )

            model.addConstr(soc["tes"][t] <= soc_nom["tes"] * dT_relative + resSC,
        ...
        for t in time_steps:
            ...
            model.addConstr(soc[dev][t] == (1-k_loss) * soc_prev +
                dt * (node["devs"][dev]["eta_bat"] * p_ch[dev][t]  - 1 / node["devs"][dev]["eta_bat"] * p_dch[dev][t] ),
