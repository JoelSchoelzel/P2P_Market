...
def compute_out_P2P(options, options_DG, par_rh, decentral_opti_results, params,
    ...
    for n in range(options["nb_bes"]):
        results_ch["power_feed_pv_bes"][n] = []
        results_ch["power_feed_chp_bes"][n] = []
        results_ch["power_demand_bes"][n] = []
        results_ch["gas_from_grid_bes"][n] = []

    for i in range(par_rh["n_opt"]):
        #for t in range(par_rh["time_steps"][i][0], par_rh["time_steps"][i][par_rh["block_duration"][0]]):
        t = par_rh["time_steps"][i][0]
        results_ch["power_from_grid"].append(sum(trade_res[i]["el_from_grid"][n] for n in range(
            options["nb_bes"])))  # sum of el bought from grid
        results_ch["power_to_grid"].append(sum(trade_res[i]["el_to_grid"][n] for n in range(
            options["nb_bes"])))  # sum of el sold to grid
        results_ch["gas_from_grid"].append(sum(decentral_opti_results[i][17][n][t] for n in range(
            options["nb_bes"])))  # sum of gas_sum over all buildings
        results_ch["power_feed"].append(sum(
            sum(decentral_opti_results[i][8][n][dev][t] for dev in ("chp", "pv")) for n in
            range(options["nb_bes"])))  # sum of p_sell over all buildings and devs
        results_ch["power_demand"].append(sum(decentral_opti_results[i][4][n][t] for n in range(
            options["nb_bes"])))  # sum of p_imp over all buildings
        results_ch["power_pv_gen"].append(
            sum(decentral_opti_results[i][8][n]["pv"][t] for n in range(options["nb_bes"])))
        results_ch["power_traded"].append(sum(trade_res[i]["el_to_distr"][n] for n in range(
            options["nb_bes"])))
        results_ch["cost_el"].append(sum(trade_res[i]["cost"][n] for n in range(
            options["nb_bes"])))
        results_ch["revenue_el"].append(sum(trade_res[i]["revenue"][n] for n in range(
            options["nb_bes"])))
